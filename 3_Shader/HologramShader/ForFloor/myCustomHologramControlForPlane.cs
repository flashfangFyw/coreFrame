...
public class myCustomHologramControlForPlane: MonoBehaviour {
   ...
   void Start()
    ...
    public void CreatHologram()
    {
        ...
        newMaterialList[i].SetFloat("_Size", (maxValue-minValue)/5);
    ...
    private void SetVerticesStartAndEnd()
    {
        ...
        {
            maxValue = transform.TransformPoint(mesh.bounds.max).x;
            minValue = transform.TransformPoint(mesh.bounds.min).x;
        }
        else
        {
