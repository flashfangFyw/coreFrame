...
Shader "Custom/myCustomHologramWithRimShaderForPlane2"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR{
				...
				float2 uv_n = UnpackNormal(tex2D(_NormalMap, i.normalDir)).rg;
				...
				half alpha=1;
					if (_EffectTime < i.posWorld.x)
					{
						alpha = 0;
					}
					else
					{
						half falloff = saturate((_EffectTime- i.posWorld.x)*_Size);
						falloff = saturate(pow(falloff, _FallOff));
